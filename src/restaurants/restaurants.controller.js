...
async function listAverageRatingByOwner(req, res, next) {
  // your solution here
  // Receive an array filled with objects
  const theData = await service.listAverageRatingByOwner();
  // Loop through array and take all avg keys and change 
  // their values to numbers.
  theData.forEach((currentData) => {
    currentData.avg = Number(currentData.avg)
  });
